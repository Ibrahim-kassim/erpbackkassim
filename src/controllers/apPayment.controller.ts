...
import { createAPPaymentSchema, sendAPPaymentVendorMessageSchema, updateAPPaymentSchema } from '../validators/apPayment.schema';
import * as service from '../services/apPayment.service';
...
export const getOutstandingBills = asyncHandler(async (req: Request, res: Response) => {
    const data = await service.getOutstandingBills(req.params.vendorId, req.tenantId!);
    res.status(200).json({ data });
});

export const create = asyncHandler(async (req: Request, res: Response) => {
    const validation = createAPPaymentSchema.safeParse(req.body);
...
export const update = asyncHandler(async (req: Request, res: Response) => {
    const validation = updateAPPaymentSchema.safeParse(req.body);
    if (!validation.success) {
        res.status(400).json({ code: 'VALIDATION_ERROR', message: 'Invalid inputs', details: validation.error.format() });
        return;
    }
...
export const sendVendorMessage = asyncHandler(async (req: Request, res: Response) => {
    const validation = sendAPPaymentVendorMessageSchema.safeParse(req.body);
    if (!validation.success) {
        res.status(400).json({ code: 'VALIDATION_ERROR', message: 'Invalid inputs', details: validation.error.format() });
        return;
    }
