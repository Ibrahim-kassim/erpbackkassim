...
const persistUploadIfNeeded = (file: Express.Multer.File): string => {
    const storedFilename = resolveStoredFilename(file);

    // Disk storage path already exists and was generated by multer.
    if (file.filename && findExistingDocumentPath(storedFilename)) {
        return storedFilename;
    }

    // Memory storage compatibility: write the buffered file to uploads folder.
    if ((file as any).buffer && Buffer.isBuffer((file as any).buffer)) {
        const targetRoot = getUploadsRoots()[0];
        if (!fs.existsSync(targetRoot)) fs.mkdirSync(targetRoot, { recursive: true });
        const targetPath = path.join(targetRoot, storedFilename);
...
export const uploadDocuments = asyncHandler(async (req: Request, res: Response) => {
    ...
    const newAttachments = files.map((file) => {
        const storedFilename = persistUploadIfNeeded(file);
        return {
